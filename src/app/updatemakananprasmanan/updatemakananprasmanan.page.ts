...
@Component({
  selector: 'app-updatemakananprasmanan',
  templateUrl: './updatemakananprasmanan.page.html',
  styleUrls: ['./updatemakananprasmanan.page.scss'],
})
export class UpdatemakananprasmananPage implements OnInit {
 ...
 namaMakananb = "";
  ...
  selectMakanan(id) {
    this.items = [];
    var sampleData = [];
    this.http.post('http://'+this.loginservice.host+'/kevyn/tampilsatumakananprasmanan.php',{'id':id},{})
      .then(data => {
  ...
  getPostData(id,namaMakanan,namaMakananb){
      ...
      this.presentLoading(true);
  		this.http.post('http://'+this.loginservice.host+'/kevyn/updatemakananprasmanan.php',{'id':id,'namaMakananb':namaMakananb},{})
  		.then(data => {
  			if(data.data == 'gagal'){
